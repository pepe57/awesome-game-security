...
Complements user-mode EtwTi syscall loggers such as [[etw-syscall-monitor]] and Instrumentation Callback loggers such as [[instrumentation-callback-syscall-logger]] by hooking at the kernel **AltSystemCallHandlers** path instead of ETW or Ring3 callbacks. Handler mechanics research PoCs such as [[win-alt-syscall-handler]] document registration limits, dispatch conditions, and thread debug-flag path selection for the same API surface. Defensive analysts can pair its per-process syscall traces with origin checks such as [[syscall-detect]] when studying custom syscall stubs vs expected `ntdll` paths.
...
[[overviews/windows-kernel]] · [[overviews/anti-cheat]] · [[etw-syscall-monitor]] · [[instrumentation-callback-syscall-logger]] · [[win-alt-syscall-handler]] · [[hidden-syscall-monitoring]] · [[kernelmon]] · [[syscall-detect]] · [[etwti-syscall-hook]]
